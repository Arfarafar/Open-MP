...
#pragma omp parallel for shared(a, b, c, chunk) private(i, tid) schedule(static, chunk)
    {
        tid = omp_get_thread_num();  // после оmp for должен следовать фор, по этому перенесем 
        for (i = 0; i < N; i++)
        {
...
*/
    ...
    {
        tid = omp_get_thread_num(); //перенесем объявление до цикла, чтобы каждый процесс знал свой номер. 
